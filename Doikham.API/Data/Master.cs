...
namespace Doikham.API.Data
{
    ...
    public class Master : IMaster
    {
        ...
        {
            string queryStr = "";
            queryStr = "insert into productgroup(ProductGroupID,ProductGroupCode,ProductGroupName,ShopID,ProductGroupActivate) select((select case when max(ProductGroupID) is null then 0 else max(ProductGroupID) end from productgroup) +ROW_NUMBER() OVER(ORDER BY bb.ProductGroupId)) AS ProductGroupID, aa.ProductDeptCode, aa.ProductDeptCode As ProductDeptName, 2 As ShopID,1 As ProductDeptActivate  from(select ProductDeptCode from SAPBOne_MasterData group by ProductDeptCode) aa left join productgroup bb on aa.ProductDeptCode = bb.ProductGroupCode where bb.ProductGroupCode is null";
            await Task.Run(() => _dbHelper.ExecuteNonQuery(queryStr, connection, transaction));

            queryStr = "update aa set aa.ProductGroupID=bb.ProductGroupID from SAPBOne_MasterData aa join  productgroup bb on aa.ProductDeptCode=bb.ProductGroupCode";
            await Task.Run(() => _dbHelper.ExecuteNonQuery(queryStr, connection, transaction));
        ...
        private async Task CreateProductPrice(DataTable dtPeriodDate, SqlConnection connection, SqlTransaction transaction)
        {
            ...
            queryStr = $"delete from productpricegroupshop; insert into productpricegroupshop(PriceGroupID,ShopID) select 1,ShopID from shop_data where ShopID>2;";
            await Task.Run(() => _dbHelper.ExecuteNonQuery(queryStr, connection, transaction));
        ...
        private async Task CreateMaterial(SqlConnection connection, SqlTransaction transaction)
        {
            ...
            queryStr = "update aa set aa.Deleted=bb.Deleted from materials aa join (select aa.MaterialID, aa.MaterialCode, case when bb.Activate = 1 then 0 else 1 end As Deleted from  materials aa inner join SAPBOne_MasterData bb on bb.ProductName7 = aa.MaterialCode group by aa.MaterialID,aa.MaterialCode,bb.Activate) bb on aa.MaterialID = bb.MaterialID";
            await Task.Run(() => _dbHelper.ExecuteNonQuery(queryStr, connection, transaction));
